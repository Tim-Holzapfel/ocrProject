...
gen_capital <- function() {
  ...
  capitals <-
    admin_regions %>%
    dplyr::mutate(
      cities = stringr::str_extract(capitals, "(?<=ct(\\.:|:))[^|]*")
    ) %>%
    dplyr::select(region, country, continent, cities) %>%
    dplyr::distinct(cities, .keep_all = TRUE)
  ...
  for (j in stop_words) {
    ...
    capitals <- tibble::add_column(capitals, locate_stop_words)
  }


  stop_words_n <- length(stop_words)

  stop_words_subset <- capitals %>% dplyr::select(5:(4 + stop_words_n))

  min_row <- function(x) min(x, na.rm = TRUE)

  suppressWarnings(capitals$min_col <- apply(stop_words_subset, 1, min_row))

  capitals[capitals$min_col == "Inf", "min_col"] <- NA
