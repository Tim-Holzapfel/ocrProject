...
#' @title Concatenating available excel files
#'
#' @description This function should not be called by itself and is rather a
#' helper function to create the final R file
#'
#' @param input_path Path to the excel file
#' @param input_continent Continent of the region
#' @param input_startpage Starting page of the Truhart section
#' @param input_endpage Ending page of the Truhart section
#'
#' @keywords internal
gen_sheet <-
...
concat_sheets <- function() {
  ...
  excel_sheets <-
    purrr::pmap_dfr(input_list, con_files) %>%
    dplyr::mutate(
      N = dplyr::row_number()
    )
...
gen_header <- function() {
  ...
  header <-
    ...
    dplyr::select(ID, Period, Ruler = Ruler_ref, N)
  ...
  index_header <-
    header %>%
    dplyr::filter(
      (!is.na(ID) & is.na(Period) & is.na(Ruler)) |
        (is.na(ID) & is.na(Period) & !is.na(Ruler))
    )
