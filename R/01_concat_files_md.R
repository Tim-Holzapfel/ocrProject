...
#'
#' @param desc_path The list pointing to the path of the excel files that are to be merged
...
create_final_sheet <- function() {
  ...
  sheet_final <-
    dplyr::left_join(sheet, header[, c("Region", "Country", "N", "Page"), by = "N"])

  sheet_final$Page <-
    zoo::na.locf(sheet_final$Page, na.rm = FALSE)

  sheet_final$Page <-
    sheet_final$Page %>% as.integer()

  sheet_final$Startpage <-
    sheet_final$Startpage %>% as.integer()

  sheet_final$Endpage <-
    sheet_final$Endpage %>% as.integer()
  ...
  sheet_final <-
    sheet_final[order(sheet_final$Continent, sheet_final$Page, decreasing = FALSE), ]

  # ABBYY 15 usually inserts a white space between page breaks. These empty rows should be dropped

  index_empty <-
    which(is.na(sheet_final$ID) & is.na(sheet_final$Period) & is.na(sheet_final$Ruler))
