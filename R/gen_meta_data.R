...
gen_meta_data <- function(input_path) {
  ...
  meta_data <-
    XML::xmlValue(XML::getNodeSet(doc, "/*/dc:description"))
  ...
  if (!all(meta_data_col_names %in% c("continent", "continent_region"))) stop(wrong_meta_data_error)
  ...
  meta_data_tbl_return <-
    meta_data_tbl %>%
    tibble::add_column(dir_path = input_path)
